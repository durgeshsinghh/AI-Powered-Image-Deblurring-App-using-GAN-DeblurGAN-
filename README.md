...
## Integrating into your app (matches the synopsis's client-server architecture)

Per the synopsis's Figure 1 (Flutter client → Spring Boot backend → Python/PyTorch ML engine),
point your Spring Boot backend's REST client at this service's `/deblur` endpoint, forward the
multipart upload it receives from the Flutter app, and return the PNG bytes straight back to the
client. Alternatively, if the Flutter app talks to this API directly, use `/deblur/base64`, since
mobile HTTP clients often find base64 JSON simpler than multipart streaming.
